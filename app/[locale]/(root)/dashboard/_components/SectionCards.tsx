...
// 💡 Importation du hook de traduction
import { useScopedI18n } from '@/locales/client'; 

interface SectionCardsProps {
    authorId: string; // L'ID de l'auteur doit être une chaîne (String)
}

// 💡 Correction de la signature du composant : reçoit un objet de props et retourne du JSX.
export function SectionCards({ authorId }: SectionCardsProps): JSX.Element {

    // 💡 Hook de traduction ciblant une nouvelle section 'dashboard'
    const t = useScopedI18n('dashboard'); 

    // Les données ne sont chargées que si authorId est fourni, ce qui est géré dans le hook useArticlesData
    const {data: categories} = useCategory()
    const {data: type } = useType()
    const {data: articles} = useArticlesData(authorId)

    // Les données 'articles' ou 'categories' pourraient être undefined ou null pendant le chargement initial.
    // L'utilisation de `?.length` gère cela.
